...
contract CvxCrvUSDTSwapper is CvxCrvUnity {
    using SafeMath for uint256;

    uint256 private tokenid = 3;
    ...
    uint256 private depositUsdcAmount;
    ...
    constructor(address[5] memory _coins, address[2] memory _pools) {
        ...
        // usdt.approve(pools[1], type(uint256).max);
        (bool success, bytes memory data) = usdt.call(abi.encodeWithSelector(SIG_APPROVE, pools[1], type(uint256).max));
    ...
    function deposit(uint256 _amount, uint256 _leverage, uint8 mode, uint256 swap, uint _previd, uint _nextid) public isUnlock returns (bool) {
        ...
        (bool success, bytes memory data) = usdt.call(abi.encodeWithSelector(SIG_TRANSFERFROM, msg.sender, address(this), depositAmount));
        require(success && (data.length == 0 || abi.decode(data, (bool))), "reward token transfer failed");

        // usdt == usda
        swapUSDAAmount = IMetaPool(pools[1]).get_dy_underlying(3, 0, depositAmount);
        ...
        // USDT -> USDC
        depositUsdcAmount = IMetaPool(pools[1]).exchange_underlying(3, 2, depositAmount, 0, address(this));
    ...
    function _deleverage(address _debtor, uint256 swap, uint _previd, uint _nextid) internal isUnlock returns (bool) {
        ...
        if (usdcBalance > 0) {
            uint256 usdtAmount = IMetaPool(pools[1]).exchange_underlying(2, 3, usdcBalance, 0, address(this));
            // usdt.transfer(msg.sender, usdtAmount);
            (bool success, bytes memory data) = usdt.call(abi.encodeWithSelector(SIG_TRANSFER, msg.sender, usdtAmount));
            require(success && (data.length == 0 || abi.decode(data, (bool))), "usdt token transfer failed");
        }
