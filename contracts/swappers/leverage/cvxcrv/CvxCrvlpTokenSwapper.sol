...
interface ICvxCrvTokenOracle {
    function get() external view returns (uint256);
}

contract CvxCvxlpTokenSwapper is CvxCrvUnity {
    using SafeMath for uint256;

    uint256 private tokenid = 5;
    
    ICvxCrvTokenOracle public oracle;
    ...
    function setOracle(address _oracle) public isOwner returns (bool) {
        oracle = ICvxCrvTokenOracle(_oracle);
        return true;
    }
    ...
    function _deleverage(address _debtor, uint256 swap, uint _previd, uint _nextid) internal isUnlock returns (bool) {
        ...
        if (swapUsdcAmount < value) {
            ...
            // USDC -> crvToken e.g. USDC -> crvFRAX
            uint256 balanceCrvTokenAmount =  ICvxCrvPool(pools[0]).add_liquidity([0, balanceUsdcAmount], 0);
            // crvToken -> cvxcrvToken e.g. crvFRAX -> cvxcrvFRAX
            uint256 pid = IRewards(rewards).pid();
            uint256 oldCvxCrvTokenAmount = IERC20(cvxcrvToken).balanceOf(address(this));
            IDeposit(depositpool).deposit(pid, balanceCrvTokenAmount, false);
            uint256 newCvxCrvTokenAmount = IERC20(cvxcrvToken).balanceOf(address(this));
            uint256 cvxcrvTokenAmount = newCvxCrvTokenAmount.sub(oldCvxCrvTokenAmount);
            // cvxcrvToken.transfer(msg.sender, cvxcrvTokenAmount);
            (bool success, bytes memory data) = cvxcrvToken.call(abi.encodeWithSelector(SIG_TRANSFER, msg.sender, cvxcrvTokenAmount));
            require(success && (data.length == 0 || abi.decode(data, (bool))), "cvxcrvToken token transfer failed");
        } else {
