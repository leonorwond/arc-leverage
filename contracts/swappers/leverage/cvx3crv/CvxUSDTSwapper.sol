...
contract CvxUSDTSwapper is CvxUnity {
    ...
    function deposit(uint256 _amount, uint256 _leverage, uint8 mode, uint256 swap, uint _previd, uint _nextid) public isUnlock returns (bool) {
        ...
        // usdt == usda
        swapUSDAAmount = IMetaPool(pools[2]).get_dy_underlying(3, 0, depositAmount);
        ...
        totalUSDTAmount = usdtAmount.add(depositAmount);
    
        // USDT -> 3CRV
        uint256 old_balance = IERC20(threeCRV).balanceOf(address(this));
        uint256[3] memory amountsAdded = [0, 0, totalUSDTAmount];
        ...
        cvxlp3CRVAmount = newCvxlp3CRVAmount.sub(oldCvxlp3CRVAmount);
    ...
    function _deleverage(address _debtor, uint256 swap, uint _previd, uint _nextid) internal isUnlock returns (bool) {
        ...
        if (threeCRVBalance > 0) {
            uint256 oldUSDTAmount = IERC20(usdt).balanceOf(address(this));
            IThreePool(pools[0]).remove_liquidity_one_coin(threeCRVBalance, 2, 0);
            uint256 newUSDTAmount = IERC20(usdt).balanceOf(address(this));
            uint256 balance = newUSDTAmount.sub(oldUSDTAmount);
            // usdt.transfer(msg.sender, balance);
            (bool success, bytes memory data) = usdt.call(abi.encodeWithSelector(SIG_TRANSFER, msg.sender, balance));
            require(success && (data.length == 0 || abi.decode(data, (bool))), "reward token transfer failed");
        }
