...
contract CvxUSDCSwapper is CvxUnity {
    ...
    function deposit(uint256 _amount, uint256 _leverage, uint8 mode, uint256 swap, uint _previd, uint _nextid) public isUnlock returns (bool) {
        ...
        // usdc == usda
        swapUSDAAmount = IMetaPool(pools[2]).get_dy_underlying(2, 0, depositAmount);
        ...
        uint256[3] memory amountsAdded = [0, depositAmount, usdtAmount];
    ...
    function _deleverage(address _debtor, uint256 swap, uint _previd, uint _nextid) internal isUnlock returns (bool) {
        ...
        if (threeCRVBalance > 0) {
            uint256 oldUSDCAmount = IERC20(usdc).balanceOf(address(this));
            IThreePool(pools[0]).remove_liquidity_one_coin(threeCRVBalance, 1, 0);
            uint256 newUSDCAmount = IERC20(usdc).balanceOf(address(this));
            uint256 balance = newUSDCAmount.sub(oldUSDCAmount);
