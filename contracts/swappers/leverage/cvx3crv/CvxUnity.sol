...
interface IMetaPool {
    ...
    function remove_liquidity(uint256 _burn_amount, uint256[2] memory _min_amounts, address receiver) external returns (uint256[2] memory);
...
contract CvxUnity is Owner {
    using SafeMath for uint256;

    address public usda;
    address public usdt;
    address public threeCRV;
    address public cvxlp3Crv;
    address[3] public pools; // 0: threepool 1: meta pool e.g. fraxpool(frax3CRV) 2: usdapool(usda3CRV)

    address public depositpool; // booster
    address public rewards; // rewards
    address[8] public extraRewards; // [crv, cvx, fxs]
    uint256 public strategy = 0;   //  leverageManager: 0 frax 1 mim 2 usdcFrax
    ...
    function approveBooster(uint256 amount) public isOwner returns (bool) {
        IMetaPool(pools[1]).approve(depositpool, amount);
        return true;
    }

    function setPools(address[3] memory _pools) public isOwner returns (bool) {
        pools = _pools;
        return true;
    }
    ...
    function setConvex(address _booster, address _rewards, address[8] memory _extraRewards) public isOwner returns (bool) {
        ...
        IMetaPool(pools[1]).approve(depositpool, type(uint256).max);
        rewards = _rewards;
